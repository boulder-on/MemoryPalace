 ...
 * The list is also dynamically self compressing. A ListOfFloat has a chunkSize to dictate the size
...
public class ListOfFloat {
    ...
    private static final long BYTE_COUNT = Float.BYTES;

    public ListOfFloat() {
        this(128, 1.5, 2);
    }

    public ListOfFloat(int chunkSize) {
     ...
     */
    public ListOfFloat(int chunkSize, double overloadFactor, int allowedDecompressedCount) {
        if (chunkSize < 10)
     ...
     * @return This list.
     */
    public ListOfFloat add(float v) {
        elementCount++;
     ...
     * @return This list.
     */
    public ListOfFloat addAll(float[] v, int offset, int len) {
        var buff = FloatBuffer.wrap(v, offset, len);
        return addAll(buff);
    }
     ...
     * @return This list.
     */
    public ListOfFloat addAll(float[] v) {
        return addAll(FloatBuffer.wrap(v));
    }
     ...
     * @return This list.
     */
    public ListOfFloat addAll(FloatBuffer buff) {
     ...
     * @return This list.
     */
    public ListOfFloat add(int idx, float v) {
        long size = size();
        if (idx < 0 || idx > size + 1)
     ...
     */
    public float set(long idx, float v)
    {
        var size = size();
        if (idx < 0 || idx >= size)
    ...
    private class Streamer implements DoublePredicate, DoubleUnaryOperator
    {
        ...
        long idx = 1;
        IteratorFloat iterator = floatIterator();
    ...
    ListIterator<Float> listIterator(int startAt)
    {
        var c = size();
        ...
        return new ListIterator<>() {
            ...
            @Override
            public void remove() {
                ListOfFloat.this.remove(idx);
            }

            @Override
            public void set(Float aDouble) {
                ListOfFloat.this.set(idx, aDouble);
            }

            @Override
            public void add(Float aDouble) {
                ListOfFloat.this.add(aDouble);
            }
        };
    }
    ...
    {
        return new Iterator<>() {
            final IteratorFloat iterator = floatIterator();
     ...
     */
    public IteratorFloat floatIterator()
    {
        return new IteratorFloat() {
            int chunkIdx = 0;
            IteratorFloat chunkIter = chunks.get(chunkIdx).floatIterator();
    ...
    public float[] toArray() {
        ...
        var ret = new float[(int)size()];
        var buff = FloatBuffer.wrap(ret);
    ...
    public void trimToSize()
    {
        ...
        for (int n = 0; n < oldChunks.size(); ++n)
        {
            ...
            c.decompress(false);
            var buff = FloatBuffer.wrap(c.uncompressed, 0, c.insertPos);
            addAll(buff);
        }
    ...
    class Chunk implements Comparable<Long>
    {
        ...
        {
            uncompressed = new float[chunkSize];
            this.offset = offset;
            if (track)
        ...
        Chunk[] insert(int idx, float v)
        {
            ...
            {
                var dest = new float[uncompressed.length + 1];
        ...
        void getAll(FloatBuffer buff)
        {
            decompress();
            buff.put(uncompressed, 0, insertPos);
        }

        float set(long idx, float v)
        {
            decompress();
            compressed = null;
        ...
        void compress()
        {
            ...
            bb.asFloatBuffer().put(uncompressed, 0, insertPos);
            uncompressed = null;
        ...
        void decompress(boolean track)
        {
            ...
            try (var in = new InflaterInputStream(bin))
            {
                ...
                uncompressed = new float[insertPos];
                bb.asFloatBuffer().get(uncompressed);
            }
            catch (IOException ex)
            {
                ex.printStackTrace();
