...
/**
 * This class is modeled off a java.util.List object, but is instead used to carry double primitives.
 * The list is also dynamically self compressing. A ListOfDouble has a chunkSize to dictate the size
...
public class ListOfDouble {
    ...
    private static final long BYTE_COUNT = Double.BYTES;

    public ListOfDouble() {
        this(128, 1.5, 2);
    }

    public ListOfDouble(int chunkSize) {
     ...
     */
    public ListOfDouble(int chunkSize, double overloadFactor, int allowedDecompressedCount) {
        if (chunkSize < 10)
     ...
     * @return This list.
     */
    public ListOfDouble add(double v) {
        elementCount++;
     ...
     * @return This list.
     */
    public ListOfDouble addAll(double[] v, int offset, int len) {
        DoubleBuffer buff = DoubleBuffer.wrap(v, offset, len);
        return addAll(buff);
    }
     ...
     * @return This list.
     */
    public ListOfDouble addAll(double[] v) {
        return addAll(DoubleBuffer.wrap(v));
    }
     ...
     * @return This list.
     */
    public ListOfDouble addAll(DoubleBuffer buff) {
     ...
     * @return This list.
     */
    public ListOfDouble add(int idx, double v) {
        long size = size();
        if (idx < 0 || idx > size + 1)
     ...
     */
    public double set(long idx, double v)
    {
        long size = size();
        if (idx < 0 || idx >= size)
    ...
    public DoubleStream stream() {
        ...
        return DoubleStream.iterate(get(0), streamer, streamer);
    }

    /**
     * The stream for the list.
     */
    private class Streamer implements DoublePredicate, DoubleUnaryOperator
    {
        final long count = size();
        long idx = 1;
        IteratorDouble iterator = doubleIterator();
        ...
        public DoublePredicate and(DoublePredicate other) {
            return DoublePredicate.super.and(other);
        }

        @Override
        public DoublePredicate negate() {
            return DoublePredicate.super.negate();
        }

        @Override
        public DoublePredicate or(DoublePredicate other) {
            return DoublePredicate.super.or(other);
        }
    ...
    ListIterator<Double> listIterator(int startAt)
    {
        long c = size();
        ...
        return new ListIterator<>() {
            ...
            @Override
            public void remove() {
                ListOfDouble.this.remove(idx);
            }

            @Override
            public void set(Double aDouble) {
                ListOfDouble.this.set(idx, aDouble);
            }

            @Override
            public void add(Double aDouble) {
                ListOfDouble.this.add(aDouble);
            }
        };
    }
    ...
    {
        return new Iterator<>() {
            final IteratorDouble iterator = doubleIterator();
     ...
     */
    public IteratorDouble doubleIterator()
    {
        return new IteratorDouble() {
            int chunkIdx = 0;
            IteratorDouble chunkIter = chunks.get(chunkIdx).doubleIterator();
            ...
            public double next()
            {
                double ret = chunkIter.next();
                if (!chunkIter.hasNext())
                {
                    ++chunkIdx;
                    if (chunkIdx < chunks.size())
                        chunkIter = chunks.get(chunkIdx).doubleIterator();
                }
                return ret;
            }
        };
    ...
    public double[] toArray() {
        ...
        var ret = new double[(int)size()];
        DoubleBuffer buff = DoubleBuffer.wrap(ret);
    ...
    public void trimToSize()
    {
        ...
        for (int n = 0; n < oldChunks.size(); ++n)
        {
            ...
            c.decompress(false);
            DoubleBuffer buff = DoubleBuffer.wrap(c.uncompressed, 0, c.insertPos);
            addAll(buff);
        }
    ...
    class Chunk implements Comparable<Long>
    {
        ...
        {
            uncompressed = new double[chunkSize];
            this.offset = offset;
            if (track)
        ...
        Chunk[] insert(int idx, double v)
        {
            ...
            double postLoad = (insertPos + 1) / (double)chunkSize;

            // We are allowed to grow, so grow and insert
            if (postLoad <= overloadFactor)
            {
                var dest = new double[uncompressed.length + 1];
        ...
        void getAll(DoubleBuffer buff)
        {
            decompress();
            buff.put(uncompressed, 0, insertPos);
        }

        double set(long idx, double v)
        {
            decompress();
            compressed = null;
        ...
        double remove(long idx)
        {
            decompress();
            compressed = null;

            double ret = uncompressed[(int)idx];
        ...
        void compress()
        {
            ...
            ByteBuffer bb = ByteBuffer.allocate(insertPos * (int)BYTE_COUNT);
            bb.order(ByteOrder.nativeOrder());
            bb.asDoubleBuffer().put(uncompressed, 0, insertPos);
            uncompressed = null;
        ...
        void decompress(boolean track)
        {
            ...
            try (InflaterInputStream in = new InflaterInputStream(bin))
            {
                ...
                uncompressed = new double[insertPos];
                bb.asDoubleBuffer().get(uncompressed);
            }
            catch (IOException ex)
            {
                ex.printStackTrace();
