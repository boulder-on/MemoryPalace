 ...
 * The list is also dynamically self compressing. A ListOfInt has a chunkSize to dictate the size
...
public class ListOfInt {
    ...
    private static final long BYTE_COUNT = Integer.BYTES;

    public ListOfInt() {
        this(128, 1.5, 2);
    }

    public ListOfInt(int chunkSize) {
     ...
     */
    public ListOfInt(int chunkSize, double overloadFactor, int allowedDecompressedCount) {
        if (chunkSize < 10)
     ...
     * @return This list.
     */
    public ListOfInt addAll(int[] v, int offset, int len) {
        var buff = IntBuffer.wrap(v, offset, len);
        return addAll(buff);
    }
     ...
     * @return This list.
     */
    public ListOfInt addAll(int[] v) {
        return addAll(IntBuffer.wrap(v));
    }
     ...
     * @return This list.
     */
    public ListOfInt addAll(IntBuffer buff) {
     ...
     * @return This list.
     */
    public ListOfInt add(int idx, int v) {
        long size = size();
        if (idx < 0 || idx > size + 1)
    ...
    public IntStream stream() {
        ...
        return IntStream.iterate(get(0), streamer, streamer);
    }

    /**
     * The stream for the list.
     */
    private class Streamer implements IntPredicate, IntUnaryOperator
    {
        final long count = size();
        long idx = 1;
        IteratorInt iterator = intIterator();
        ...
        public IntPredicate and(IntPredicate other) {
            return IntPredicate.super.and(other);
        }
        ...
        public IntUnaryOperator compose(IntUnaryOperator before) {
            return IntUnaryOperator.super.compose(before);
        }

        @Override
        public IntUnaryOperator andThen(IntUnaryOperator after) {
            return IntUnaryOperator.super.andThen(after);
        }
    }
    ...
    ListIterator<Integer> listIterator(int startAt)
    {
        var c = size();
        ...
        return new ListIterator<>() {
            ...
            @Override
            public void remove() {
                ListOfInt.this.remove(idx);
            }

            @Override
            public void set(Integer aDouble) {
                ListOfInt.this.set(idx, aDouble);
            }

            @Override
            public void add(Integer aDouble) {
                ListOfInt.this.add(aDouble);
            }
        };
    }
    ...
    {
        return new Iterator<>() {
            final IteratorInt iterator = intIterator();
     ...
     */
    public IteratorInt intIterator()
    {
        return new IteratorInt() {
            int chunkIdx = 0;
            IteratorInt chunkIter = chunks.get(chunkIdx).floatIterator();
    ...
    public int[] toArray() {
        ...
        var ret = new int[(int)size()];
        var buff = IntBuffer.wrap(ret);
    ...
    public void trimToSize()
    {
        ...
        for (int n = 0; n < oldChunks.size(); ++n)
        {
            ...
            c.decompress(false);
            var buff = IntBuffer.wrap(c.uncompressed, 0, c.insertPos);
            addAll(buff);
        }
    ...
    class Chunk implements Comparable<Long>
    {
        ...
        {
            uncompressed = new int[chunkSize];
            this.offset = offset;
            if (track)
        ...
        Chunk[] insert(int idx, int v)
        {
            ...
            {
                var dest = new int[uncompressed.length + 1];
        ...
        void getAll(IntBuffer buff)
        {
            decompress();
            buff.put(uncompressed, 0, insertPos);
        }

        int set(long idx, int v)
        {
            decompress();
            compressed = null;
        ...
        void compress()
        {
            ...
            bb.asIntBuffer().put(uncompressed, 0, insertPos);
            uncompressed = null;
        ...
        void decompress(boolean track)
        {
            ...
            try (var in = new InflaterInputStream(bin))
            {
                ...
                uncompressed = new int[insertPos];
                bb.asIntBuffer().get(uncompressed);
            }
            catch (IOException ex)
            {
                ex.printStackTrace();
